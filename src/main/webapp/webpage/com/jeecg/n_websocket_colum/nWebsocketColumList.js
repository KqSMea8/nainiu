...
function content(strate_page){
	/*var parms="configId=n_websocket_colum&field=id,"
			+ "phoneType,phone,code,details,ipaddress,";*/
...
//	var goods_detaisl_status=document.getElementById("goods_detaisl_status").value;
	...
	//alert(shoptype_value+"="+t);
	  $.ajax({
    		...
    		type : 'POST',
    		datatype:"json",
    		url : 'nWebsocketColumController.do?datagrid',// 请求的action路径
...
function contentdetails(date_list){
	...
	console.log("dd="+date_list);
	 ...
	 var htmcontent='';
		...
		htmcontent+='<tr >	'
					  +'<td>'+intege+'</td>'
					  +'<td>'+is_null(obj.sendname)+'</td>'
					  +'<td>'+is_null(obj.fnmber)+'</td>'
			          +'<td>'+is_time(obj.createDate)+'</td>'
			          +'<td class="last">'
			        //  +'<a href="javascript:doUrl(&#39;nWebsocketColumController.do?goUpdate&amp;id='+obj.id+'&#39;)">编辑</a>'
					 // +'<a href="javascript:delDataT(&#39;nWebsocketColumController.do?doDel&amp;id='+obj.id+'&#39;)">删除</a>'
					  +'<a href="javascript:doUrl(&#39;nWebsocketColumController.do?websocketlist&amp;id='+obj.id+'&#39;)">聊天记录</a>'
					  +'</td>'
			       +'</tr>';
          }
