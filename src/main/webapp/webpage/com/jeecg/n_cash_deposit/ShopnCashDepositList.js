		...
		//	var ce=is_null(obj.ce);
			...
			htmcontent+='<tr >	'
			       ...
			       //   +'<td>'+is_null(obj.marketingname)+'</td>'
					 ...
					 // +'<a href="javascript:delDataT(&#39;nCashDepositController.do?doDel&amp;id='+obj.id+'&#39;)">删除</a>'
					  +'<a href="javascript:doUrl(&#39;nCashDepositController.do?goUpdate&amp;load=detail&amp;id='+obj.id+'&#39;)">详情</a>'
					  +'</td>'
			       +'</tr>';
          }
...
function doTrans(mid){
	//alert(mid);
	var account = $("#aliAccount").val().trim();
	if(mid==""||mid==null){
		alert("系统异常，请重新登录。");
		...
		$.ajax({
			type:"post",
			url:"nCashDepositController.do?applymoney",
			async:true,
			data:{merchantid:mid,account:account},
			dataType:"json",
			success:function(data){
				parent.layer.alert(data.msg, {
        	        icon: 1,
