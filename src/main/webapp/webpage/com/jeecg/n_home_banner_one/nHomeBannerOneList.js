...
function content(strate_page){
	var parms="configId=n_home_banner_one&field=id,createName,createBy,createDate,update_name,updateBy,update_date,sys_org_code,remarks,del_flag,bpm_status,"
			+ "title,picurl,activityurl,";
...
//	var goods_detaisl_status=document.getElementById("goods_detaisl_status").value;
	...
	//alert(shoptype_value+"="+t);
	  $.ajax({
    		...
    		type : 'POST',
    		datatype:"json",
    		url : 'nHomeBannerOneController.do?datagrid&'+parms,// 请求的action路径
	 ...
	 var htmcontent='';
		...
		//	var ce=is_null(obj.ce);
			...
			htmcontent+='<tr >	'
			          ...
			          +'<td>'+is_null(obj.activityurl)+'</td>'
			          +'<td>'+is_time(obj.createDate)+'</td>'
			          +'<td class="last">'
			          +'<a href="javascript:doUrl(&#39;nHomeBannerOneController.do?goUpdate&amp;id='+obj.id+'&#39;)">编辑</a>'
					  +'<a href="javascript:delDataT(&#39;nHomeBannerOneController.do?doDel&amp;id='+obj.id+'&#39;)">删除</a>'
					  +'<a href="javascript:doUrl(&#39;nHomeBannerOneController.do?goUpdate&amp;load=detail&amp;id='+obj.id+'&#39;)">详情</a>'
					  +'</td>'
			       +'</tr>';
          }
