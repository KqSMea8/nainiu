...
function content(strate_page){
	var parms="configId=n_goods_detaisl&field=id,createName,createBy,createDate,update_name,updateBy,update_date,sys_org_code,remarks,del_flag,bpm_status,"
			+ "goodsDetaislStatus,merchantId,goodsClassifyId,title,duanTitle,starttype,weight,bazaarPrice,contents,sendTime,post,returns,fine,picOne,standard,standardOne,standardTwo,sumNumbers,codes,groupPurchase,setBounds,xiangou,reject,orders,shangpintype,joinHuodong,carousel,details,goodsone,goodstwo,goodsthree,standardOnelist,standardTwolist,tuanprices,unitprices,";
		...
		//	var ce=is_null(obj.ce);
			...
			htmcontent+='<tr >	'
			   ...
			   //       +'<td>'+is_time(obj.createDate)+'</td>'
			          +'<td class="last">'
			          +'<a href="javascript:doUrl(&#39;nGoodsDetaislController.do?goUpdate&amp;id='+obj.id+'&#39;)">编辑</a>'
					  +'<a href="javascript:delDataT(&#39;nGoodsDetaislController.do?doDel&amp;id='+obj.id+'&#39;)">删除</a>'
					  +'<a href="javascript:doUrl(&#39;nGoodsDetaislController.do?goUpdate&amp;load=detail&amp;id='+obj.id+'&#39;)">详情</a>'
...
function openUserSelect(id) {
	tid=id;
		...
		$.dialog({content: 'url:nGoodsDetaislController.do?orderslist', zIndex: getzIndex(), title: '用户列表', 
	...
	//	alert(tid);
		 ...
		 // alert(rowsData);
			if(eval(rowsData)>eval(0)){
				$.ajax({
				      url: 'nGoodsDetaislController.do?doOrders',
				      cache: false,
					  dataType: "json",
