...
//	var goods_detaisl_status=document.getElementById("goods_detaisl_status").value;
	...
	//alert(shoptype_value+"="+t);
	  $.ajax({
    		...
    		type : 'POST',
    		datatype:"json",
    		url : 'nPhoneCodeController.do?datagrid&'+parms,// 请求的action路径
	...
	console.log("dd="+date_list);
	 ...
	 var htmcontent='';
		...
		//	var ce=is_null(obj.ce);
			...
			var phoneType=is_null(obj.phoneType);
			...
			htmcontent+='<tr >	'
			          ...
			          +'<td>'+is_null(obj.code)+'</td>'
			          +'<td>'+is_null(obj.details)+'</td>'
			          +'<td>'+is_time(obj.createDate)+'</td>'
			          +'<td class="last">'
			        //  +'<a href="javascript:doUrl(&#39;nPhoneCodeController.do?goUpdate&amp;id='+obj.id+'&#39;)">编辑</a>'
					 // +'<a href="javascript:delDataT(&#39;nPhoneCodeController.do?doDel&amp;id='+obj.id+'&#39;)">删除</a>'
					  +'<a href="javascript:doUrl(&#39;nPhoneCodeController.do?goUpdate&amp;load=detail&amp;id='+obj.id+'&#39;)">详情</a>'
					  +'</td>'
			       +'</tr>';
          }
