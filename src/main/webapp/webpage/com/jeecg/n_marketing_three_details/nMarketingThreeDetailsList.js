...
function content(strate_page){
	var parms="configId=n_marketing_one&field=id,createName,createBy,createDate,updateName,updateBy,updateDate,sys_org_code,remarks,del_flag,bpm_status,"
			+ "merchantid,merchantname,goodsid,goodsname,audittype,shoptype,tuanprices,unitprices,numbers,orders,";
	...
	//alert(shoptype_value+"="+t);
	  $.ajax({
    		...
    		type : 'POST',
    		datatype:"json",
    		url : 'nMarketingThreeDetailsController.do?datagrid&'+parms,// 请求的action路径
		...
		if(audittype=="0"){
			...
			htmcontent+='<tr >	'
			         ...
			         // +'<td>'+is_null(obj.numbers)+'</td>'
			          ...
			          +'<td class="last">'
			          +'<a href="javascript:delaudit(&#39;nMarketingThreeDetailsController.do?doAudit&amp;audittype=1&amp;id='+obj.id+'&#39;)">审核通过</a>'
			          +'<a href="javascript:delaudit(&#39;nMarketingThreeDetailsController.do?doAudit&amp;audittype=2&amp;id='+obj.id+'&#39;)">未通过</a>'
					...
					//  +'<a href="javascript:doUrl(&#39;nMarketingThreeDetailsController.do?goUpdate&amp;load=detail&amp;id='+obj.id+'&#39;)">查看活动商品</a>'
...
function openUserSelect(id) {
	tid=id;
		...
		$.dialog({content: 'url:nMarketingThreeDetailsController.do?orderslist', zIndex: getzIndex(), title: '商品列表', 
	...
	//	alert(tid);
		 ...
		 // alert(rowsData);
			if(eval(rowsData)>eval(0)){
				...
				$.ajax({
				      url: 'nMarketingThreeDetailsController.do?doOrders',
				      cache: false,
					  dataType: "json",
