...
function content(strate_page){
	...
	//alert(shoptype_value+"="+t);
	  $.ajax({
    		...
    		type : 'POST',
    		datatype:"json",
    		url : 'nMarketingThreeController.do?datagrid&'+parms,// 请求的action路径
...
function contentdetails(date_list){
	...
	/*	var priceStatu=is_null(obj.priceStatu);
		...
		if(priceStatu=="0"){
			...
			/*var joinnumb=dictionaryarray[obj.id];
			var aduitnumb=doAuditNumberarray[obj.id];
	...
	/*	 var datenums=dictionaryarray[obj.id];
		 var aduitnumb=doAuditNumberarray[obj.id];
		
		 var joinnumb=0;
		 var aduitnumb=0;
		// alert(datenums);
		 if(datenums!=undefined){
			for ( var j = 0;j<datenums.length;j++) {
				var objnumbs = datenums[j];
			        var audittype=objnumbs.audittype;
			        var numbers=objnumbs.numbers;
			        if(audittype==0){
			        	joinnumb=numbers;
			        }
			...
			htmcontent+='<tr >	'
					...
					//  +'<td>'+priceStatu+'</td>'
			          ...
			          +'<td class="last">'
			          +'<a href="javascript:doUrl(&#39;nMarketingThreeController.do?goUpdate&amp;id='+obj.id+'&#39;)">编辑</a>'
					 // +'<a href="javascript:delData(&#39;nMarketingThreeController.do?doDel&amp;id='+obj.id+'&#39;)">删除</a>'
					  +'<a href="javascript:doUrl(&#39;nMarketingThreeController.do?goUpdate&amp;load=detail&amp;id='+obj.id+'&#39;)">详情</a>'
					  +'<a href="javascript:doUrl(&#39;nMarketingThreeDetailsController.do?list&amp;id='+obj.id+'&#39;)">查看活动商品</a>'
					  +'</td>'
			       +'</tr>';
          }
