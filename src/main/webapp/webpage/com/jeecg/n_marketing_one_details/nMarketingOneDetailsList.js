	 ...
	 var htmcontent='';
		...
		if(audittype=="0"){
			...
			htmcontent+='<tr >	'
			          ...
			          +'<td class="last">'
			          +'<a href="javascript:delaudit(&#39;nMarketingOneDetailsController.do?doAudit&amp;audittype=1&amp;id='+obj.id+'&#39;)">审核通过</a>'
			          +'<a href="javascript:delaudit(&#39;nMarketingOneDetailsController.do?doAudit&amp;audittype=2&amp;id='+obj.id+'&#39;)">未通过</a>'
...
function openUserSelect(id) {
	tid=id;
		...
		$.dialog({content: 'url:nMarketingOneDetailsController.do?orderslist', zIndex: getzIndex(), title: '商品列表', 
	...
	//	alert(tid);
		...
		//  alert(rowsData);
			if(eval(rowsData)>eval(0)){
				...
				$.ajax({
				      url: 'nMarketingOneDetailsController.do?doOrders',
				      cache: false,
					  dataType: "json",
