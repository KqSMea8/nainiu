...
function content(strate_page){
	var parms="configId=n_home_banner&field=id,createName,createBy,createDate,update_name,updateBy,update_date,sys_org_code,remarks,del_flag,bpm_status,"
			+ "merchantId,orderId,phone,cardnum,merchantId,orderId,phone,cardnum,"
			+ "gameState,errorCode,result,sendType,reason,";
	...
	//alert(shoptype_value+"="+t);
	  $.ajax({
    		...
    		type : 'POST',
    		datatype:"json",
    		url : 'nPhoneRechargeController.do?datagrid&'+parms,// 请求的action路径
	...
	console.log("dd="+date_list);
	 ...
	 var htmcontent='';
		for ( var i = 0;i<date_list.length;i++) {
			...
			var intege=i+1;
			var obj = date_list[i];
			var sendType=is_null(obj.sendType);
		...
		/*	var aftersaleStatusname="";
			...
			htmcontent+='<tr >	'
					  ...
					  +'<td>'+is_null(obj.cardnum)+'</td>'
					  +'<td>'+is_null(obj.errorCode)+'</td>'
			          +'<td>'+sendTypename+'</td>'
			          +'<td>'+is_null(obj.reason)+'</td>'
			          +'<td>'+is_time(obj.createDate)+'</td>'
			          +'<td class="last">'
			          +'<a href="javascript:doUrl(&#39;nPhoneRechargeController.do?pt_goUpdate&amp;id='+obj.id+'&#39;)">详情</a>'
	                  +'</td>'
			       +'</tr>';
          }
