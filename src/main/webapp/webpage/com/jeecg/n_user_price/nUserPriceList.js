...
function content(strate_page){
	var parms="configId=n_home_banner&field=id,createName,createBy,createDate,update_name,updateBy,update_date,sys_org_code,remarks,del_flag,bpm_status,"
			+ "userId,realname,userPriceType,price,orderid,goodsid,goodsname,";
	var realname=document.getElementById("realname").value;
	var userPriceType=document.getElementById("userPriceType").value;
...
//	var goods_detaisl_status=document.getElementById("goods_detaisl_status").value;
	...
	//alert(shoptype_value+"="+t);
	  $.ajax({
    		...
    		type : 'POST',
    		datatype:"json",
    		url : 'nUserPriceController.do?datagrid&'+parms,// 请求的action路径
	...
	//console.log("dd="+date_list);
	 ...
	 var htmcontent='';
		...
		//	var ce=is_null(obj.ce);
			var userPriceType=is_null(obj.userPriceType);
		
			if(userPriceType=="0"){
				userPriceType="提现";
			} else if(userPriceType=="1"){
				userPriceType="充值";
			}else{
				userPriceType="退款";
		    }
			...
			htmcontent+='<tr >	'
			          ...
			          +'<td class="last">'
//			          +'<a href="javascript:doUrl(&#39;nUserPriceController.do?goUpdate&amp;id='+obj.id+'&#39;)">编辑</a>'
					 // +'<a href="javascript:delDataT(&#39;nUserPriceController.do?doDel&amp;id='+obj.id+'&#39;)">删除</a>'
					  +'<a href="javascript:doUrl(&#39;nUserPriceController.do?goUpdate&amp;load=detail&amp;id='+obj.id+'&#39;)">详情</a>'
					  +'</td>'
			       +'</tr>';
          }
