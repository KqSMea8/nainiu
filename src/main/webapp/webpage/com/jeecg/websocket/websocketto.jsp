	         ...
	         formData.append("file",$("#"+id)[0].files[0]);
	                 ...
	                 /*  websocket.send(JSON.stringify({
	                   ...
	                   socket.send(JSON.stringify({
	                       ...
	                       ,data: {"msg":mine.content,"from":sendid,"to":acceptid,"fromName":"mine.username","toName":"to.username"}
	                   }));
	                  },
	                  error: function (data) {
	                      alert("文件上传出错！");
...
//     var src = document.getElementById("img").src;//获取当前点击的pimg元素中的src属性    
 ...
 //var id="1";
    ...
    if ('WebSocket' in window) {
    	socket = new WebSocket("ws://192.168.51.108:8080/jeecg/WebSockete/"+socketid);
    }
    else {
  ...
  /*  function closeWebSocket() {
   ...
   /*     websocket.send(JSON.stringify({
        ...
        socket.send(JSON.stringify({
            type: 'friend' //随便定义，用于在服务端区分消息类型
            ,data: {"msg":message,"from":sendid,"to":socketid,"fromName":"mine.username","toName":"to.username"}
        }));
    }
