...
function content(strate_page){
	...
	//alert(shoptype_value+"="+t);
	  $.ajax({
    		...
    		type : 'POST',
    		datatype:"json",
    		url : 'nMarketingMemberDetailsController.do?datagrid&'+parms,// 请求的action路径
	 ...
	 var htmcontent='';
		...
		if(audittype=="0"){
			...
			htmcontent+='<tr >	'
			          ...
			          +'<td class="last">'
			          +'<a href="javascript:delaudit(&#39;nMarketingMemberDetailsController.do?doAudit&amp;audittype=1&amp;id='+obj.id+'&#39;)">审核通过</a>'
			          +'<a href="javascript:delaudit(&#39;nMarketingMemberDetailsController.do?doAudit&amp;audittype=2&amp;id='+obj.id+'&#39;)">未通过</a>'
					...
					//  +'<a href="javascript:doUrl(&#39;nMarketingMemberDetailsController.do?goUpdate&amp;load=detail&amp;id='+obj.id+'&#39;)">查看活动商品</a>'
...
function openUserSelect(id) {
	tid=id;
		...
		$.dialog({content: 'url:nMarketingMemberDetailsController.do?orderslist', zIndex: getzIndex(), title: '商品列表',
...
//		  alert(rowsData);
			if(eval(rowsData)>eval(0)){
				...
				$.ajax({
				      url: 'nMarketingMemberDetailsController.do?doOrders',
				      cache: false,
					  dataType: "json",
