...
function content(strate_page){
	var parms="configId=n_evaluate&field=id,createName,createBy,createDate,update_name,updateBy,update_date,sys_org_code,remarks,del_flag,bpm_status,"
			+ "goodsId,title,userId,realname,usernameurl,detaisl,miaosu,logistics,serve,evaluatepic,evaluateStatus,";
	var title=document.getElementById("title").value;
	var realname=document.getElementById("realname").value;
	var evaluateStatus=document.getElementById("evaluateStatus").value;
...
//	var goods_detaisl_status=document.getElementById("goods_detaisl_status").value;
	...
	//alert(shoptype_value+"="+t);
	  $.ajax({
    		...
    		type : 'POST',
    		datatype:"json",
    		url : 'nEvaluateController.do?datagrid&'+parms,// 请求的action路径
	...
	console.log("dd="+date_list);
	 ...
	 var htmcontent='';
		...
		//	var ce=is_null(obj.ce);
			...
			var evaluate_status=is_null(obj.evaluateStatus);
			...
			htmcontent+='<tr >	'
			          ...
			          +'<td>'+is_null(obj.miaosu)+'星</td>'
			          +'<td>'+is_null(obj.logistics)+'星</td>'
			          +'<td>'+is_null(obj.serve)+'星</td>'
			          +'<td>'+evaluate_status+'</td>'
			          +'<td>'+is_time(obj.createDate)+'</td>'
			          +'<td class="last">'
			          +'<a href="javascript:doUrl(&#39;nEvaluateController.do?goUpdate&amp;id='+obj.id+'&#39;)">编辑</a>'
					  +'<a href="javascript:delDataT(&#39;nEvaluateController.do?doDel&amp;id='+obj.id+'&#39;)">删除</a>'
					  +'<a href="javascript:doUrl(&#39;nEvaluateController.do?goUpdate&amp;load=detail&amp;id='+obj.id+'&#39;)">详情</a>'
					  +'</td>'
			       +'</tr>';
          }
