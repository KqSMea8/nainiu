...
function content(strate_page){
	var parms="configId=n_home_banner&field=id,createName,createBy,createDate,update_name,updateBy,update_date,sys_org_code,remarks,del_flag,bpm_status,"
			+ "userId,realname,merchantId,goodsId,couponId,couponName,money,startTime,endTime,details,couponType,fullMoney,grantType,couponstatus,";
...
//	var goods_detaisl_status=document.getElementById("goods_detaisl_status").value;
	...
	//alert(shoptype_value+"="+t);
	  $.ajax({
    		...
    		type : 'POST',
    		datatype:"json",
    		url : 'nUserCouponController.do?datagrid&'+parms,// 请求的action路径
	...
	console.log("dd="+date_list);
	 ...
	 var htmcontent='';
		for ( var i = 0;i<date_list.length;i++) {
			var intege=i+1;
			var obj = date_list[i];
			var couponType=is_null(obj.couponType);
			...
			htmcontent+='<tr >	'
			          ...
			          +'<td>'+is_null(obj.coupon_name)+'</td>'
			          +'<td>'+is_null(obj.money)+'</td>'
			          +'<td>'+is_time(obj.startTime)+'</td>'
			          +'<td>'+is_time(obj.endTime)+'</td>'
			          +'<td>'+couponType+'</td>'
			          +'<td>'+is_time(obj.createDate)+'</td>'
			          +'<td class="last">'
			        //  +'<a href="javascript:doUrl(&#39;nUserCouponController.do?goUpdate&amp;id='+obj.id+'&#39;)">编辑</a>'
					 // +'<a href="javascript:delDataT(&#39;nUserCouponController.do?doDel&amp;id='+obj.id+'&#39;)">删除</a>'
					  +'<a href="javascript:doUrl(&#39;nUserCouponController.do?goUpdate&amp;load=detail&amp;id='+obj.id+'&#39;)">详情</a>'
					  +'</td>'
			       +'</tr>';
          }
